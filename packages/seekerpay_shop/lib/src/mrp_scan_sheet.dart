...
class _MrpScanSheetState extends ConsumerState<MrpScanSheet> with SingleTickerProviderStateMixin {
  ...
  final _nameCtrl = TextEditingController(); final _barcodeCtrl = TextEditingController(); final _localPriceCtrl = TextEditingController(); double? _confirmedUsd; bool _isConfirmed = false; String? _capturedPath;
  static const _kFName = 'name', _kFBarcode = 'barcode', _kFPrice = 'price', _kFExpiry = 'expiry';
  final _rescanFields = <String>{}; bool _rescanningFields = false; bool _isRescanMode = false; final _rescanModeFields = <String>{}; String _rescanNameBackup = '', _rescanBarcodeBackup = '';

  @override void initState() { super.initState(); _initCamera(); }
  @override void dispose() { if (!_isConfirmed && _capturedPath != null) { try { File(_capturedPath!).deleteSync(); } catch (_) {} } _focusTimer?.cancel(); _controller?.dispose(); _nameCtrl.dispose(); _barcodeCtrl.dispose(); _localPriceCtrl.dispose(); super.dispose(); }
  ...
  Future<void> _capture() async {
    ...
    try {
      ...
      print(' [Scanner] >>> CAPTURED PHOTO: $originalPath');

      String? detectedBarcode;
      final barcodeScanner = BarcodeScanner();
      try {
        final barcodes = await barcodeScanner.processImage(InputImage.fromFilePath(originalPath));
        if (barcodes.isNotEmpty) {
          detectedBarcode = barcodes.first.rawValue;
          print(' [Scanner] >>> BARCODE FOUND: $detectedBarcode');
        } else {
          print(' [Scanner] >>> NO BARCODE detected.');
        }
      } catch (e) { print(' [Scanner] >>> Barcode scan ERROR: $e'); }
      finally { barcodeScanner.close(); }
      ...
      if (detectedBarcode != null) {
        ...
        if (match != null) {
          print(' [Scanner] >>> Instant match in local history.');
          _nameCtrl.text = match.name; _barcodeCtrl.text = match.barcode;
          ...
          if (mounted) _populateFields(_mrpData!); return;
        }
        
        // 2. API Lookup
        final prefs = await SharedPreferences.getInstance();
        final barcodeKey = prefs.getString('spay_barcode_lookup_key');
        final barcodeEnabled = prefs.getBool('spay_barcode_lookup_enabled') ?? false;
        
        print(' [Scanner] >>> CONFIG: KeyLoaded=${barcodeKey != null}, Enabled=$barcodeEnabled');
        
        final service = ProductLookupService(barcodeLookupApiKey: barcodeKey, enabled: barcodeEnabled);
        final apiProduct = await service.lookup(detectedBarcode);
        
        if (apiProduct != null) {
          print(' [Scanner] >>> API Success: ${apiProduct.name}');
          _nameCtrl.text = apiProduct.name; _barcodeCtrl.text = apiProduct.barcode;
          setState(() { _mrpData = MrpData(productName: apiProduct.name, barcode: apiProduct.barcode, mrpAmount: apiProduct.lastPriceUsd, currencyCode: 'USD'); _status = _Status.result; });
          if (mounted) _populateFields(_mrpData!); return;
        }
      }

      // ── OCR Fallback if no barcode or no API result ──
      ...
      if (image != null) {
        ...
        ocrPaths.add(p1); _capturedPath = p1;
        ...
        ocrPaths.add(p4);
      } else { _capturedPath = originalPath; ocrPaths.add(originalPath); }
      ...
      if (combinedRaw.trim().length >= _kMinOcrChars && await LocalLlmService.isEnabled()) {
        ...
        if (llmData != null) { data = llmData.copyWith(barcode: detectedBarcode); }
        else { data = MrpParser.parse(results[0]).copyWith(barcode: detectedBarcode); }
      } else {
  ...
  Future<void> _fetchRate(MrpData data) async {
    final code = data.currencyCode ?? 'USD';
    if (code == 'USD') { 
      setState(() { _rateToUsd = 1.0; _convState = _ConversionState.done; }); 
      if (data.hasPrice) { 
        _localPriceCtrl.text = data.mrpAmount!.toStringAsFixed(2); 
        setState(() => _confirmedUsd = data.mrpAmount); 
      } return; 
    }
    setState(() => _convState = _ConversionState.loading);
    try {
      final rate = await CurrencyConverter.rateToUsd(code); if (!mounted) return;
      if (rate == null) throw Exception('Rate not found');
      setState(() { _rateToUsd = rate; _convState = _ConversionState.done; });
      if (data.hasPrice) {
        _localPriceCtrl.text = data.mrpAmount!.toStringAsFixed(2);
        setState(() => _confirmedUsd = data.mrpAmount! / rate);
      }
  ...
  void _confirm() {
    _isConfirmed = true; final name = _nameCtrl.text.trim(); final barcode = _barcodeCtrl.text.trim();
    if (name.isEmpty || _confirmedUsd == null || _confirmedUsd! <= 0) return;
    final product = Product(barcode: barcode.isNotEmpty ? barcode : 'scanned_${DateTime.now().millisecondsSinceEpoch}', name: name, brand: _mrpData?.brand ?? 'Scanned via AI', category: 'Scanned', imageUrl: _capturedPath != null ? 'file://$_capturedPath' : null, quantity: _mrpData?.quantity, expiryDate: _mrpData?.expDate, lastPriceUsd: _confirmedUsd, savedAt: DateTime.now());
    widget.onConfirm(product, _confirmedUsd!); Navigator.of(context).pop();
  }

  void _reset() { if (_capturedPath != null) { try { File(_capturedPath!).deleteSync(); } catch (_) {} _capturedPath = null; } setState(() { _status = _Status.camera; _mrpData = null; _rawOcrText = ''; _llmRawOutput = ''; _usedLlm = false; _errorMessage = null; _nameCtrl.clear(); _barcodeCtrl.clear(); _localPriceCtrl.clear(); _confirmedUsd = null; _isConfirmed = false; _isRescanMode = false; _rescanFields.clear(); }); }
  ...
  Widget _cameraScreen() {
    ...
    return Stack(children: [
      ...
      Positioned(bottom: 0, left: 0, right: 0, child: _bottomBar(isTorch)),
    ]);
  }
  ...
  Widget _resultScreen() {
    ...
    return Column(children: [
      ...
      Expanded(child: SingleChildScrollView(padding: const EdgeInsets.fromLTRB(20, 16, 20, 32), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          Container(
            ...
            child: Row(children: [
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('${mrp.currencySymbol}${mrp.mrpAmount!.toStringAsFixed(2)}', style: const TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.w900)),
                Text(mrp.currencyCode!, style: const TextStyle(color: Colors.white38, fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 1)),
              ]),
              const Padding(padding: EdgeInsets.symmetric(horizontal: 16), child: Icon(Icons.arrow_forward_rounded, color: Colors.white24, size: 18)),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                if (_rateToUsd != null) Text('\$${(mrp.mrpAmount! / _rateToUsd!).toStringAsFixed(2)}', style: const TextStyle(color: _kPrimary, fontSize: 24, fontWeight: FontWeight.w900))
                else const Text('...', style: TextStyle(color: _kPrimary, fontSize: 24, fontWeight: FontWeight.w900)),
                const Text('USD EQUIVALENT', style: TextStyle(color: Colors.white38, fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 1)),
              ])),
        ...
        _label('SET PRICE ($code)'), const SizedBox(height: 6),
        TextField(
          controller: _localPriceCtrl, 
          keyboardType: const TextInputType.numberWithOptions(decimal: true), 
          autofocus: !hasData, 
          style: const TextStyle(color: _kPrimary, fontSize: 32, fontWeight: FontWeight.w900, letterSpacing: -1), 
          decoration: InputDecoration(
            hintText: '0.00', hintStyle: const TextStyle(color: Colors.white24, fontSize: 32, fontWeight: FontWeight.w900), 
            prefixText: '$symbol ', prefixStyle: const TextStyle(color: _kPrimary, fontSize: 32, fontWeight: FontWeight.w900), 
            enabledBorder: const UnderlineInputBorder(borderSide: BorderSide(color: Colors.white12)), 
            focusedBorder: const UnderlineInputBorder(borderSide: BorderSide(color: _kPrimary, width: 1.5)),
            suffixIcon: _convState == _ConversionState.loading ? const SizedBox(width: 20, height: 20, child: Center(child: CircularProgressIndicator(strokeWidth: 2, color: _kPrimary))) : null,
          ), 
          onChanged: _onLocalPriceChanged,
        ),
        if (_confirmedUsd != null && code != 'USD') Padding(padding: const EdgeInsets.only(top: 6), child: Row(children: [
          const Icon(Icons.swap_horiz_rounded, color: Colors.white38, size: 12),
          const SizedBox(width: 6),
          Text('≈ \$${_confirmedUsd!.toStringAsFixed(2)} USD', style: const TextStyle(color: Colors.white38, fontSize: 12, fontWeight: FontWeight.w700)),
        ])),
