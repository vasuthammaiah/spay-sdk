...
class _ShopLlmSettingsState extends State<ShopLlmSettings> {
  ...
  double _progress = 0;
  bool _testing = false; String? _testResult;
  ...
  Future<void> _startDownload() async {
    ...
    if (mounted) setState(() { _progress = 0; _testResult = null; });
    try {
  ...
  Future<void> _deleteModel() async {
    ...
    if (conf == true) { await LocalLlmService.deleteModel(); if (mounted) setState(() { _enabled = false; _testResult = null; _progress = 0; for (final k in _Step.values) { _status[k] = _StepStatus.idle; _detail.remove(k); } }); }
  }
        ...
        Padding(padding: const EdgeInsets.fromLTRB(16, 14, 16, 16), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          if (_allOk) ...[
            ...
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text('AI PLAYGROUND', style: TextStyle(color: Colors.white.withOpacity(0.3), fontSize: 9, fontWeight: FontWeight.w900, letterSpacing: 2)),
              GestureDetector(onTap: () => setState(() => _showRules = !_showRules), child: Text(_showRules ? 'HIDE RULES' : 'SHOW RULES', style: TextStyle(color: _kPrimary.withOpacity(0.5), fontSize: 8, fontWeight: FontWeight.w900, letterSpacing: 1))),
            ]),
            if (_showRules) ...[
              const SizedBox(height: 8),
              Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: Colors.blue.withOpacity(0.05), borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.blue.withOpacity(0.2))),
                child: const Text(
            ...
            TextField(controller: _playgroundCtrl, style: const TextStyle(color: Colors.white, fontSize: 13), maxLines: 3, decoration: InputDecoration(hintText: 'Enter sample label text to test AI...', hintStyle: const TextStyle(color: Colors.white24, fontSize: 12), filled: true, fillColor: Colors.white.withOpacity(0.03), border: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide.none))),
            const SizedBox(height: 10),
...
class _ClaudeVisionSettingsState extends State<ClaudeVisionSettings> {
  static const _kPrimary = Color(0xFFFFEB3B), _kSurface = Color(0xFF1A1A1A), _kGreen = Color(0xFF00E676);
    ...
    return Container(margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8), decoration: BoxDecoration(color: _kSurface, borderRadius: BorderRadius.circular(12)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        ...
        Padding(padding: const EdgeInsets.fromLTRB(16, 14, 16, 16), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          if (_editing) ...[
            TextField(controller: _ctrl, obscureText: _obscure, autofocus: true, style: const TextStyle(color: Colors.white, fontSize: 13, fontFamily: 'monospace'), decoration: InputDecoration(hintText: 'sk-ant-api03-...', hintStyle: const TextStyle(color: Colors.white24, fontSize: 12), filled: true, fillColor: Colors.white.withOpacity(0.05), border: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: const BorderSide(color: Colors.white12)), enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: const BorderSide(color: Colors.white12)), focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide(color: _kPrimary.withOpacity(0.6))), contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 12), suffixIcon: IconButton(icon: Icon(_obscure ? Icons.visibility_rounded : Icons.visibility_off_rounded, color: Colors.white38, size: 18), onPressed: () => setState(() => _obscure = !_obscure)))),
            const SizedBox(height: 10),
